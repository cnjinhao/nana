...
namespace nana
{
	...
	namespace drawerbase::toolbar
	{
		...
		public:
			...
			void insert(size_type pos, std::string text, const nana::paint::image& img, tool_type type)
			{
				...
				cont_.insert(cont_.begin() + (pos < cont_.size() ? pos : cont_.size()), m);
			}
	...
	//class toolbar
		...
		toolbar::item_proxy toolbar::append(const std::string& text, const nana::paint::image& img)
		{
...
#ifdef __cpp_char8_t
		toolbar::item_proxy toolbar::append(std::u8string_view text, const nana::paint::image& img)
		{
			get_drawer_trigger().items().push_back(to_string(text), img);
			api::refresh_window(handle());
			return {this, get_drawer_trigger().items().size() - 1u};
		}
#endif
...
#ifdef __cpp_char8_t
		void toolbar::toggle_group(size_type index, std::u8string_view group)
		{
			toggle_group(index, to_string(group));
		}
#endif
