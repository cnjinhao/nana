...
namespace nana
{
	...
	//class label
		...
		label::label(window parent, std::wstring_view text, bool visible)
		{
			create(parent, rectangle{}, visible);
			caption(text);
		}
...
#ifdef __cpp_char8_t
		::nana::size label::measure(paint::graphics& graph, std::u8string_view str, unsigned allowed_width_in_pixel, bool format_enabled, align h_align, align_v v_align)
		{
			drawerbase::label::renderer rd;
			rd.format(format_enabled);
			rd.parse(to_wstring(str));
		...
		void label::_m_complete_creation()
		{
