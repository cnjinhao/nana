...
#include "br_place_parts.hpp"
#include <iostream>

namespace nana
{
	struct badname: br_place::error
	{
		explicit badname(  ::std::string           what,
							const br_place&           plc,
							const char*            name   = "unknown",
							std::string::size_type pos    = std::string::npos)
			:br_place::error(what + ": bad field name '" + (name ? name : "nullptr") + "'.",
				          plc, (name ? name : "nullptr"), pos)
		{}
	};
	typedef br_place_parts::number_t number_t;
	typedef br_place_parts::repeated_array repeated_array;

	namespace br_place_parts
	{
		class tokenizer
		{
		public:
	...
	struct br_place::implement
	{
		...
		struct error : std::invalid_argument
		{
			explicit error(std::string    what,
				...
				: std::invalid_argument{ what + " from br_place implementation " },
				  pos{ pos },
		...
		{
			dock_position position{ dock_position::up };
		...
		std::unique_ptr<division> scan_div(br_place_parts::tokenizer&, bool implicitly_started, const std::string& ignore_duplicate = {});
		...
		bool is_vertical(dock_position position);

		void hide_pane_indicators();
	...
	class br_place::implement::field_gather
		: public br_place::field_interface
	{
	public:

        /// \todo introduce a br_place::implement::field_gather::error ??
		struct element_t
		{
			window handle;
		...
		explicit field_gather(br_place * p) noexcept
			: place_ptr_(p)
		{}
	...
	private:
		void _m_insert_widget(window wd, bool to_fasten)  /// \todo better errors caption of failed windows, field
		{
			if (api::empty_window(wd))
				throw br_place::error("Failed to insert an invalid window handle.", *place_ptr_);

			if (api::get_parent_window(wd) != place_ptr_->window_handle())
				throw br_place::error("Failed to insert a window which is not a child of the br_place-binded window", *place_ptr_);
		...
		{
			return static_cast<field_interface*>(this)->operator<<(br_agent<label>(label_text ? label_text : ""));
		}

		field_interface& operator<<(std::string label_text) override
		{
			return static_cast<field_interface*>(this)->operator<<(br_agent<label>(label_text));
		}
		...
		void _m_add_agent(const detail::br_agent& ag) override
		{
			this->operator<<(
	...
	class br_place::implement::field_dock
	{
	public:
		div_dockpane * attached{ nullptr };					//attached div object
		std::unique_ptr<br_place_parts::dockarea> dockarea;	//the dockable widget
	...
	public:
		...
		std::pair<double,double> calc_weight_floor(window wd)
		{
			...
			if (this->display)
			{
				...
				if ((fit_policy::none != this->fit) && this->field)
				{
					...
					if (fit_count > 1)
					{
						...
						for (std::size_t i = 0; i < fit_count - 1; ++i)
						{
							...
							{
								double precise_px = 0;
								fv += gap_value.get_value(100, 0, precise_px, dm, false);
							}
						}
				...
				if (!(this->weight.empty() || this->is_percent()))
				{
		...
		static double limit_px(const division* div, double px, unsigned area_px, const br_place_parts::display_metrics& dm) noexcept
		{
			...
			if (!div->min_px.empty())
			{
				auto v = div->min_px.get_value(static_cast<int>(area_px), dm);
			...
			if (!div->max_px.empty())
			{
				auto v = div->max_px.get_value(static_cast<int>(area_px), dm);
				if (px > v)
					return v;
			}
			return px;
		...
		nana::rectangle margin_area(const br_place_parts::display_metrics& dm) const
		{
			return margin.area(field_area, dm);
		}
	...
	public:
		...
		br_place_parts::margin	margin;
		br_place_parts::repeated_array gap;
	...
	class br_place::implement::div_arrange
		: public division
	{
	public:
		div_arrange(bool vert, std::string&& name, br_place_parts::repeated_array&& arr) noexcept
		...
		{
			return kind::arrange != kind_of_division;
		}
		...
		void collocate(window wd) override
		{
			...
			auto area_margined = margin_area(dm);
			rectangle_rotator area(vert, area_margined);
			auto area_px = area.w();

			auto fa = _m_fixed_and_adjustable(kind_of_division, area_px, dm); // fixed pixels and number of adjustable items

			double adjustable_px = _m_revise_adjustable(fa, area_px, dm); // amount of pixels for each adjustable item 
			...
			for (auto& child_ptr : children)					/// First collocate child div's !!!
			{
				...
				else
				{
					...
					{
						if (child->weight.empty() || child->is_fixed())
							child->weight.assign_percent((child_px / area_px) * 100);
					}
				...
				for (auto & el : field->elements)
				{
					...
					if (fit_policy::none != this->fit)
					{
						...
						if (run_.fit_extents.end() != i)
						{
							...
							if (vert)
								move_r.x += br_place_parts::differ(area_margined.width, move_r.width) / 2;
							else
								move_r.y += br_place_parts::differ(area_margined.height, move_r.height) / 2;
					...
					if (!moved)
					{
						px = static_cast<unsigned>(arrange_.at(index).get_value(area_px, adjustable_px, precise_px, dm, true));
						element_r.w_ref() = px;
					...
					if (index + 1 < field->elements.size())
						position += (px + gap.at(index).get_value(area_px, 0, precise_px, dm, true));
	...
	private:
		static std::pair<unsigned, std::size_t> _m_calc_fa(const br_place_parts::number_t& number, unsigned area_px, double& precise_px, const br_place_parts::display_metrics& dm)
		{
		...
		std::pair<unsigned, std::size_t> _m_fixed_and_adjustable(kind match_kind, unsigned area_px, const br_place_parts::display_metrics& dm) const noexcept
		{
		...
		double _m_revise_adjustable(std::pair<unsigned, std::size_t>& fa, // fixed pixels and number of adjustable items
			                        unsigned area_px, 
			                        const br_place_parts::display_metrics& dm)
		{
			...
			for (auto& child : children)
			{
				...
				if (!child->min_px.empty())
					min_px = child->min_px.get_value(static_cast<int>(area_px), dm);
				...
				if (!child->max_px.empty())
					max_px = child->max_px.get_value(static_cast<int>(area_px), dm);
	...
	private:
		br_place_parts::repeated_array arrange_;
	};

	class br_place::implement::div_grid
		: public division
	{
	public:
		div_grid(std::string&& name, br_place_parts::repeated_array&& arrange, std::vector<rectangle>&& collapses) noexcept
		...
		void collocate(window wd) override
		{
			...
			const br_place_parts::display_metrics dm{ wd };
			auto const area = margin_area(dm);
			auto const gap_size = static_cast<unsigned>(gap.at(0).get_value(area.width, dm)); //gap_size is 0 if gap isn't specified
					...
					for (std::size_t u = 0; (u < edge && i != end); ++u)
					{
						...
						for (std::size_t v = 0; (v < edge && i != end); ++v, ++i)
						{
							...
							if (arr.empty())
								...
								value = static_cast<decltype(value)>(arr.get_value(static_cast<int>(area.width), dm));
	...
	class br_place::implement::div_splitter
		: public division
	{
		struct div_block
		{
		...
		enum{splitter_px = 4};
	public:
		div_splitter(const br_place_parts::number_t & init_weight, implement* impl) noexcept :
			...
			if (splitter_.empty())
			{
				...
				auto grab_fn = [this, wd](const arg_mouse& arg)
				{
					...
					else if (event_code::mouse_move == arg.evt_code)
					{
						...
						auto area_px = rectangle_rotator(vert, div_owner->margin_area(dm)).w();
						double imd_rate = 100.0 / area_px;
						left_px = static_cast<int>(limit_px(leaf_left, left_px, area_px, dm));
						...
						right_px = static_cast<int>(limit_px(leaf_right, right_px, area_px, dm));
	...
	private:
		...
		void _m_update_div(std::string& div)
		{
			...
			if (div.npos == tag_pos)
				throw br_place::implement::error("please report an issue: the splitter was unable to update division " + div, name);
		...
		rectangle_rotator _m_update_splitter_range(const br_place_parts::display_metrics& dm)
		{
			const bool vert = (cursor::size_ns == splitter_cursor_);

			rectangle_rotator area(vert, div_owner->margin_area(dm));
			...
			if (!leaf_left->min_px.empty())
				pos += static_cast<int>(leaf_left->min_px.get_value(area.w(), dm));

			if (!leaf_left->max_px.empty())
				endpos = left_base + static_cast<int>(leaf_left->max_px.get_value(area.w(), dm));

			if (!leaf_right->min_px.empty())
				endpos = (std::min)(right_base - static_cast<int>(leaf_right->min_px.get_value(area.w(), dm)), endpos);

			if (!leaf_right->max_px.empty())
				pos = (std::max)(right_base - static_cast<int>(leaf_right->max_px.get_value(area.w(), dm)), pos);
	...
	private:
		...
		br_place_parts::splitter	splitter_;
		...
		br_place_parts::number_t init_weight_;
	};

	class br_place::implement::div_dockpane
		: public division, public br_place_parts::dock_notifier_interface
	{
	public:
		...
		void notify_move() override
		{
			...
			else
			{
				...
				if (pane)
				{
					...
					//if (impl_ptr_->to_dock(this, pane, dock_position::tab))
					//{
					//	impl_ptr_->collocate();
					//}
				}
	...
	private:
		bool _m_hit_window() const
		{
			...
			return rectangle{ pos, api::window_size(impl_ptr_->window_handle) }.is_hit(api::cursor_position());
		}
	public:
	...
	class br_place::implement::div_dock
		: public division
	{
	...
	public:
		...
		void collocate(window wd) override
		{
			...
			for (auto & child : children)
			{
				...
				if (!child->weight.empty())
				{
					weight = child->weight.get_value(is_vert ? room.height : room.width, dm);
					if (weight > room_px)
						weight = room_px;
				}
				else
	...
	class br_place::implement::div_switchable
		: public division
	{
	public:
	...
	private:
		void collocate(window wd) override
		{
			...
			for (auto & child : children)
			{
				...
				{
					div = child.get();
					div->field_area = this->margin_area(dm);
					div->collocate(wd);
					break;
				}
			}
			...
			rectangle empty_r{ this->margin_area(dm).position() , size{ 0, 0 } };
			for (auto & child : children)
			{
				if (child.get() != div)
				{
	...
	{
		pane_indicators.emplace_back(new dock_indicator{ dock_position::up });
			...
			for (size_t i = 0; i < div->children.size(); ++i)
			{
				...
				else if (division::kind::arrange == div->children[i]->kind_of_division)
				{
					...
					else if (div->children[i]->children.size() == 1)
					{
						...
						child->weight = div->children[i]->weight;
	...
	void br_place::implement::collocate()
	{
	...
	br_place::implement::division * br_place::implement::search_div_name(division* start, const std::string& name) noexcept
	{
		if (start)
		{
			if (start->name == name)
	...
	static int get_parameter(br_place_parts::tokenizer& tknizer, std::size_t pos)
	{
		...
		throw br_place_parts::tokenizer::error("the type of the " + std::string{pos_strs[pos]} +" parameter for collapse should be integer.", tknizer);
	}

	/// Parse div-text
	/*
	 * @param implicitly_started indicates whether the field in div-text starts without < mark.
	 * @param ignore_duplicate indicates whether a field is allowed to have a name which is same as its ancestor.
	 */
	auto br_place::implement::scan_div(br_place_parts::tokenizer& tknizer, bool implicitly_started, const std::string& ignore_duplicate) -> std::unique_ptr<division>
	{
		using token = br_place_parts::tokenizer::token;
		...
		br_place_parts::repeated_array fit_parameters;

		//These variables stand for the new division's attributes
		std::string name;
		number_t weight, min_px, max_px;
		br_place_parts::repeated_array arrange, gap;
		br_place_parts::margin margin;
		...
		try {
			...
			const br_place_parts::display_metrics dm{ this->window_handle };
			...
			if ((!min_px.empty()) && (!max_px.empty()) && (min_px.get_value(100, dm) > max_px.get_value(100, dm))) //use a fixed dpi value for comparing two values.
			{
				min_px.reset();
				max_px.reset();
			}
			...
			if ((!min_px.empty()) && (!weight.empty()) && (weight.get_value(100, dm) < min_px.get_value(100, dm)))
				weight.reset();

			if ((!max_px.empty()) && (!weight.empty()) && (weight.get_value(100, dm) > max_px.get_value(100, dm)))
				weight.reset();
		...
		catch (br_place::error& ) { throw; }
		catch (       error& ) { throw; }
		catch (br_place_parts::tokenizer::error& e)
		{
	...
	void br_place::implement::check_unique(const division* div) const
	{
		...
		check_fn = [&check_fn, &unique, &tmp](const implement::division* div)
		{
			if (!div->name.empty())
			{
				...
				if (ptr)
					throw std::invalid_argument("br_place, the name '" + div->name + "' is redefined.");
				ptr = &tmp;
			}
	...
	//connect the field/dock with div object,
	void br_place::implement::connect(division* start)
	{
		if (!start)
			return;
	...
	void br_place::implement::disconnect() noexcept
	{
		for (auto & fd : fields)
		{
	...
	bool br_place::implement::is_vertical(dock_position position)
	{
		return (dock_position::up == position || dock_position::down == position) ? true : false;
	}

	void br_place::implement::hide_pane_indicators()
	{
		for (auto& i : pane_indicators)
			i->docker.reset();
	}

	void br_place::implement::hide_indicators()
	{
		hide_pane_indicators();
		if (tab_indicator.dock_area)
		{
			tab_indicator.dock_area.reset();
			tab_indicator.graph.release();
		}
	}

	void br_place::implement::show_pane_indicators(division* div)
	{
		if (!div)
			return;
	...
	bool br_place::implement::hit_indicators(dock_position* position) const
	{
		for (auto& i : pane_indicators)
		{
			if (!i->docker)
	...
	br_place::implement::division* br_place::implement::hit_panes_or_dock()
	{
		if (!root_division || division::kind::dockpane == root_division->kind_of_division)
		{
			std::cout << "null root division or kind = dockpane" << std::endl;
			return nullptr;
		}
	...
	br_place::implement::division* br_place::implement::hit_dock()
	{
		point pos;
		API::calc_screen_point(window_handle, pos);
		if (rectangle{ pos + root_division->field_area.position(), root_division->field_area.dimension() }.is_hit(API::cursor_position()))
			return root_division.get();
		return nullptr;
	}
	...
	bool br_place::implement::to_float(division* div)
	{
		...
		for (size_t i = 0; i < owner->children.size(); ++i)
		{
			if (owner->children[i].get() == div)
			{
				...
				floating_divs.back()->weight.reset();
	...
	bool br_place::implement::to_dock(division* div, division* where_div, dock_position position)
	{
		...
		if (!root_division)
		{
			//todo: add dock as root division first before div_dockpane
			root_division.reset(div);
			floating_divs.erase(floating_divs.begin() + i_div);
			return true;
		}
		else
		{
			if (position == dock_position::tab)
			{
				if (root_division->kind_of_division == implement::division::kind::dock)
				{
					...
					else
					{
						if (where_div)
						{
							...
							//auto dockarea= divDockPane->dockable_field->dockarea.release();

							//divDockPane->dockable_field = wheredivDockPane->dockable_field;
			...
			else if (root_division->kind_of_division == implement::division::kind::dock)
			{
				if (root_division->children.size() == 0) {
					div->div_owner = root_division.get();
					div->div_next = nullptr;
				...
				else
				{
					...
					if (root_division->children[0]->kind_of_division == implement::division::kind::dockpane)
					{
	...
	window br_place::window_handle() const
	{
		return impl_->window_handle;
	}

	void br_place::splitter_renderer(std::function<void(window, paint::graphics&, mouse_action)> fn)
	{
	...
	void br_place::div(std::string div_text)
	{
		try
		{
			br_place_parts::tokenizer tknizer(div_text.c_str());
			impl_->disconnect();
		...
		catch (br_place::implement::error & e)
		{
	...
	const std::string& br_place::div() const noexcept
	{
	...
	void br_place::modify(const char* name, const char* div_text)
	{
		if (nullptr == div_text)
		...
		try
		{
			br_place_parts::tokenizer tknizer(div_text);
	...
	br_place::field_reference br_place::field(const char* name)
	{
		if (!valid_field_name(name))
	...
	void br_place::field_visible(const char* name, bool vsb)
	{
		if (!valid_field_name(name))
	...
	bool br_place::field_visible(const char* name) const
	{
		if (!valid_field_name(name))
			throw badname("field_visible()", *this, name);
	...
	void br_place::field_display(const char* name, bool dsp)
	{
		if (!valid_field_name(name))
	...
	bool br_place::field_display(const char* name) const
	{
		if (!valid_field_name(name))
			throw badname("field_display()", *this, name);
	...
	void br_place::collocate()
	{
		impl_->collocate();
	}

	void br_place::erase(window handle)
	{
	...
	br_place& br_place::dock(const std::string& name, std::string factory_name, std::function<std::unique_ptr<widget>(window)> factory)
	{
		...
		if (factory_name.empty() && dock_ptr->attached)
		{
			...
			{
				dock_ptr->dockarea.reset(new ::nana::br_place_parts::dockarea);
	...
	widget* br_place::dock_create(const std::string& factory)
	{
		auto i = impl_->dock_factoris.find(factory);
		if (i == impl_->dock_factoris.end())
			throw std::invalid_argument("nana::br_place::dock_create - invalid factory name(" + factory + ") of dockpane");
	...
	widget* br_place::add_pane(std::string dock_id, std::function<std::unique_ptr<widget>(window)> factory, const std::string& caption, const std::string& relative_pane_id, dock_position dock_position)
	{
		...
		implement::division* where_div = nullptr;
		if (!relative_pane_id.empty())
		{
			std::function<implement::division* (implement::division* div)> hit_fn;
			hit_fn = [&hit_fn, relative_pane_id](implement::division* div) -> implement::division*
			{
				if (implement::division::kind::dockpane == div->kind_of_division)
				{
					if (div->name == relative_pane_id)
						return div;
				}
	...
	br_place::error::error(const std::string& what,
						const br_place& plc,
						std::string field,
						std::string::size_type pos)
