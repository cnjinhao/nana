...
#ifndef NANA_GUI_BR_PLACE_HPP
#define NANA_GUI_BR_PLACE_HPP
...
namespace nana
{
	...
	class br_agent
		: public detail::br_agent
	{
	public:
		explicit br_agent(std::function<void(Widget&)> initializer)
			: init_(std::move(initializer))
		{}

		explicit br_agent(const char* text)
			: text_(text)
		{
			throw_not_utf8(text);
		}

		explicit br_agent(std::string text, std::function<void(Widget&)> initializer = {})
		...
		: ::nana::noncopyable
	{
        ...
        public:
			...
			template<typename Widget>
			field_interface& operator<<(const br_agent<Widget>& ag)
			{
				_m_add_agent(ag);
				return *this;
			}
		private:
			virtual void _m_add_agent(const detail::br_agent&) = 0;
		};
	public:
		...
		br_place();
		br_place(window);///< Attaches to a specified widget.
		~br_place();
		...
		br_place& dock(const std::string& dockname, const std::string& factory_name, Args&& ... args)
		{
		...
		br_place& dock(const std::string& dockname, std::string factory_name, std::function<std::unique_ptr<widget>(window)> factory);
...
#endif //#ifndef NANA_GUI_BR_PLACE_HPP
